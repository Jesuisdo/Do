...
def filtrer_groupes_avec_gagnant(X_rel, y, course_ids):
    ...
    ou EXACTEMENT UN candidat du shortlist VERT est le vrai gagnant
    (somme(y)==1 dans le groupe de 5 candidats) -- une cible degeneree (0
    partout) n'apprend rien sur 'qui gagne', et un groupe avec plusieurs
    gagnants (dead-heat / ex-aequo officiel, rare mais reel dans les
    donnees de courses hippiques) ne definit pas UNE cible binaire propre
    non plus. Les deux cas sont donc exclus de l'ENTRAINEMENT uniquement
    (jamais de l'EVALUATION, qui reste sur la population VAL_CALIB
    complete). Verification stricte et TRANSPARENTE du protocole : le
    nombre de groupes a gagnant unique / sans gagnant / a gagnants
    multiples est compte et rapporte explicitement (aucun assert qui
    ferait planter le run silencieusement sur un phenomene reel de
    donnees) ; un garde-fou signale toutefois une anomalie si la part de
    groupes a gagnants multiples est anormalement elevee (> 2%), ce qui
    indiquerait alors un vrai bug d'alignement plutot que de rares
    ex-aequo."""
    ...
    somme_par_course_unique = d.groupby("course_id")["y"].sum()
    n_courses_total = somme_par_course_unique.shape[0]
    n_courses_zero = int((somme_par_course_unique == 0).sum())
    n_courses_un = int((somme_par_course_unique == 1).sum())
    n_courses_multi = int((somme_par_course_unique > 1).sum())
    part_multi = n_courses_multi / n_courses_total if n_courses_total else 0.0
    lib.log(f"    Repartition des groupes (avant filtre) : {n_courses_un} avec exactement 1 gagnant, "
            f"{n_courses_zero} sans gagnant dans le Top-5, {n_courses_multi} avec PLUSIEURS gagnants "
            f"(dead-heat/ex-aequo officiel, {round(100*part_multi,2)}%).")
    assert part_multi <= 0.02, (
        f"ANOMALIE : {round(100*part_multi,2)}% des groupes ont plusieurs gagnants -- part anormalement "
        "elevee pour de simples ex-aequo, verifier un eventuel bug d'alignement course_id/cible.")
    masque = somme_par_course == 1
    return X_rel[masque], d.loc[masque, "y"].values, masque, n_courses_zero, n_courses_multi
...
def main():
    ...
    X_train_spec_rel_f, y_train_spec_f, masque_train, n_courses_train_zero, n_courses_train_multi = filtrer_groupes_avec_gagnant(
    ...
    lib.log(f"  TRAIN : {len(X_train_spec_rel_f)} lignes / {n_courses_train_apres} courses conservees pour "
            f"l'entrainement (gagnant unique present dans le Top-5) -- {n_courses_train_zero} courses exclues "
            f"(gagnant hors Top-5, cible non exploitable) + {n_courses_train_multi} courses exclues "
            f"(plusieurs gagnants -- dead-heat/ex-aequo officiel, cible non exploitable pour une classification "
            f"binaire propre).")
    lib.log(f"  Verification protocole : repartition des groupes rapportee explicitement ci-dessus (garde-fou "
            f"a 2% valide, sinon le script se serait arrete en erreur).")
