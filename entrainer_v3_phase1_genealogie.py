# -*- coding: utf-8 -*-
"""
entrainer_v3_phase1_genealogie.py -- PHASE 1/2 de la piste 3 (genealogie),
demandee par Dorian le 28/08/2026 apres avoir retenu B (lambdarank_graded)
comme meilleur candidat de la piste 1, tout en constatant que 71,6% des
courses restent "toujours ratees" quelle que soit la formulation de
l'objectif de ranking (aucune nouvelle feature testee jusqu'ici).

Copie STRICTE de entrainer_v3_phase1.py (chargement, resolution
d'identite, 109+22 variables v3 point-in-time, decoupage chronologique
70/15/15 identique) -- AUCUNE modification de la construction des
variables v3 existantes, pour garantir que le baseline "v3-gagnant" et le
candidat "B" restent exactement reproductibles a l'identique des runs
precedents. Le SEUL ajout est la construction, sur les MEMES lignes deja
chronologiquement triees, des 20 variables de genealogie point-in-time
(voir variables_genealogie.py : statistiques pere/mere avec lissage
bayesien hierarchique, volume d'observations, aptitude distance/terrain).
Ni le modele de reference (v2/v3-gagnant), ni B, ne sont recalcules ici --
seule la phase 2 entraine les modeles, a partir du checkpoint produit ici.

Ne se connecte a Supabase QU'EN LECTURE (meme REQUETE PLAT-only que
v3_lib.py). N'ecrit rien en base.
"""
import gc
import pickle
...
def main():
    ...
    lib.log("=" * 100)
    lib.log("PISTE 3 -- PHASE 1/2 -- CHARGEMENT, VARIABLES v3 + GENEALOGIE POINT-IN-TIME, SPLIT -- 28/08/2026")
    lib.log("=" * 100)

    lib.log("\n[1/7] Chargement des donnees brutes depuis Supabase (PLAT, toutes lignes, y compris nom_pere/nom_mere)...")
    ...
    lib.log("\n[4/7] Construction des 20 variables de genealogie point-in-time (piste 3, NOUVEAU)...")
    features_geneal = construire_variables_genealogie(lignes_triees)
    df_geneal = pd.DataFrame(features_geneal)
    assert len(df_geneal) == len(df), "desalignement genealogie vs variables v3 -- meme liste triee attendue"
    ...
    # --- decoupage chronologique STRICT, identique a v2/v3 : 70/15/15 ---
    ...
    # (le jeu TEST n'est pas construit ici : cette piste reste, comme demande,
    #  au stade VALIDATION uniquement -- aucun TEST A/B lance.)
    del df
    gc.collect()

    lib.log("\n[6/7] Decoupage chronologique strict (identique a v2/v3, memes bornes de courses) :")
    ...
    # + genealogie ensemble, controle sur TRAIN uniquement) ---
    ...
    X_train_v3_geneal = pd.concat(
        [X_train_v3.reset_index(drop=True), X_train_geneal_seul.reset_index(drop=True)], axis=1)
    X_val_v3_geneal = pd.concat(
        [X_val_v3.reset_index(drop=True), X_val_geneal_seul.reset_index(drop=True)], axis=1)
    ...
    lib.log(f"   Matrice v3+genealogie (B+genealogie) : {X_train_v3_geneal.shape[1]} colonnes "
            f"({len(colonnes_geneal_filtrees)} nouvelles variables de genealogie).")

    # --- groupes pour l'objectif de ranking (B/lambdarank_graded), meme
    # convention que entrainer_v3_phase1_ranking.py : contiguite garantie
    # par le tri ["date_course", "course_id"] ci-dessus. ---
    course_id_train = df_train["course_id"].reset_index(drop=True)
    df_val_complet = df_val.copy()

    del X_train_geneal_seul, X_val_geneal_seul
    gc.collect()
    ...
    lib.log(f"\n[7/7] Checkpoint sauvegarde : {CHECKPOINT_PATH} "
            f"(X_train_v3={X_train_v3.shape}, X_train_v3_geneal={X_train_v3_geneal.shape}, "
            f"X_val_v3={X_val_v3.shape}, X_val_v3_geneal={X_val_v3_geneal.shape}, df_val={df_val_complet.shape})")
    lib.log("\nPHASE 1 (piste 3) TERMINEE -- la phase 2 peut entrainer v3-gagnant, B et B+genealogie depuis ce checkpoint.")
